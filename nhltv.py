...
class NHLTV(Plugin):
    ...
    def _login(self, email, password):
        auth = "Basic {}".format(
            base64.urlsafe_b64encode(_NHL_WEBAPP_VER).decode("ascii")
        )
        headers = {
                ...
                for epg in media.get("epg", []):
                    ...
                    if title in ("NHLTV", "Audio"):
                        for item in epg.get("items", []):
                            ...
                            log.info(
                                "Found {} {} broadcast feed ({})".format(
                                    feed_type, broadcast_type, feed_name
                                )
                            )
    ...
    def _get_session_key(self, event_id):
        ...
        if not session_key:
            ...
            raise PluginError(
                "Could not obtain session key: {}".format(json.get("status_message"))
            )
    ...
    def _get_streams_for_feed(self, feed, audio_only=False):
        ...
        if json.get("status_code") != 1:
            log.debug(
                "Could not get streams for {}/{}: {}".format(
                    event_id, content_id, json.get("status_message")
                )
            )
            return streams
        ...
        for event in json.get("user_verified_event", []):
            for content in event.get("user_verified_content", []):
                for media in content.get("user_verified_media_item", {}):
                    ...
                    if (
                        media.get("blackout_status", {}).get("status")
                        != "SuccessStatus"
                    ):
                        msg = (
                            "This content is unavailable in your region due to NHL blackout restrictions."
                    ...
                    if url:
                        ...
                        if audio_only:
                            ...
                            name_fmt = None
                        parsed = HLSStream.parse_variant_playlist(
                            self.session, url, name_fmt=name_fmt, name_prefix=prefix
                        )
    ...
    def _get_streams(self):
        ...
        if self.options.get("prefer_french"):
            log.info(
                "French language broadcast will be preferred when it is available."
            )
        ...
        if prefer_team:
            ...
            if prefer_team in NHL_TEAMS:
                ...
                log.info(
                    "{} home/away broadcast will be preferred when it is available.".format(
                        team
                    )
                )
                ...
                log.info(
                    "Unknown team {}. Valid choices for --nhltv-prefer-team are:".format(
                        prefer_team
                    )
                )
